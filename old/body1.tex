...
This catalogue documents threats posed by unvetted workloads running at
...
A workload is ``unvetted'' if it arrived through a path that establishes
no basis for trusting it --- the operating system's package manager did
not validate it, no review vouched for it, and it may or may not be
signed: npm, PyPI, or crates packages, container images from public
registries, AI agent binaries, MCP servers,
\texttt{curl\ \textbar{}\ sh} installers, AI-generated code being
executed locally. The catalogue uses ``workload'' generically. Where a
threat's realisation differs significantly between workload classes, the
entry notes the variants.
...
Out of scope & X1--X10 & Threats a user-level confinement approach
...
\textbf{Definition.} An unvetted workload reads credentials, command
...
\textbf{Reconnaissance target enumeration.} A representative list of the
paths a credential-hunting workload attempts to read, by category, is
given in Appendix B.
...
for the semantic-review limit, X10) in Part 2; a confinement reduces but
...
\hypertarget{t1.12-display-server-the-host-session-leg}{%
\section{T1.12 --- Display server: the host-session
leg}\label{t1.12-display-server-the-host-session-leg}}
...
\hypertarget{t1.13-abstract-socket-namespace-escape-via-a-shared-network-namespace}{%
\section{T1.13 --- Abstract-socket namespace escape via a shared network
namespace}\label{t1.13-abstract-socket-namespace-escape-via-a-shared-network-namespace}}
...
vulnerability) require human review. Catalogued as X10 in Part 2.
